...
const YouTube = 0,
  Nico = 1,
  SoundCloud = 2;
const videoName = ["YouTube", "ニコニコ動画", "SoundCloud"];
let g_list, g_idx;
...
$.get("https://rpgen3.github.io/player/sample.txt", (r) => {
  ...
  window.inputURL = rpgen3.addInputText(
    "#hideArea",
    Object.assign(
      {
        save: "動画URLリスト入力欄",
...
const makeKeyOfPlaylist = (videoType, id) =>
  `playlist#${videoName[videoType]}#${id}`;
    ...
    new Array(3)
      ...
      .forEach((videoType) => {
        rpgen3
          .getSaveKeys()
          .filter((v) =>
            new RegExp(`^${selectSaveDataType()}#${videoType}#`).test(v)
          )
          .forEach((v) => {
...
function loadList() {
  ...
  Promise.all(
    ...
    .then((result) => {
      ...
      const firstFunc = [playFirstYouTube, playFirstNico, playFirstSoundCloud];
      const promiseArray = [];
      if (!g_firstLoadedFlags.every((v) => v)) {
        g_list.forEach((v) => {
          ...
          promiseArray.push(
            new Promise((resolve) => firstFunc[v[0]](v[1], resolve))
          );
        });
      }
...
function makeElm({ resolve, h, ttl, userName, img }) {
  ...
  if (userName) {
    infoElm = infoElm.add(
      $("<div>").prependTo(h).text(userName).css({
        top: 5,
        fontSize: 10,
        color: "#cccccc",
...
function loadItem({ timeStamp, v, i, id, cover, funcList, h }) {
  new Promise((resolve, reject) => {
    const makeElm2 = ({ ttl, userName, img }) =>
        makeElm({ resolve, h, ttl, userName, img }),
      keyOfVideoInfo = `videoInfo#${videoName[v[0]]}#${id}`;
    switch (v[0]) {
      case YouTube: {
        const img = `https://i.ytimg.com/vi/${id}/hqdefault.jpg`;
        if (!showInfoOfYouTubeFlag())
          return resolve($("<img>").attr({ src: img }));
        setCache({
          key: keyOfVideoInfo,
          callback: makeElm2,
          getData: (save) => {
    ...
    .then((elm) => {
      ...
      new Promise((resolve, reject) => {
        ...
        .finally(() => {
          ...
          hHideArea.empty();
          const next = i + 1;
          if (next < funcList.length)
            setTimeout(() => funcList[next](), (60 / 130) * 1000);
        });
    })
    .catch((err) => msg(err, true));
}
...
function analyzeCmd(s) {
  const num = "([0-9]+(\\.[0-9]+)?)",
    ...
    ss = [s, ...[mStart, mEnd, mRate].filter((v) => v)].reduce((a, v) =>
      a.replace(v[0], "")
    );
  let start,
    end,
    rate = mRate ? Number(mRate[1]) : 100;
  if (!mStart && !mEnd) {
...
function judgeURL(str) {
  ...
  switch (d[1] + "." + d[0]) {
    ...
    case "youtube.com":
      ...
      if (p.list && /playlist/.test(url))
        return (resolve) => getPlaylistYT(resolve, "playlist", p.list);
      if (/user/.test(url)) {
      ...
      if (/channel/.test(url)) {
        ...
        if (m)
          return (resolve) => getPlaylistYT(resolve, "playlist", "UU" + m[1]);
      }
...
function setNico(id, start = 0) {
  return iframes[Nico].find("iframe")
    .attr({
...
function playFirstSoundCloud(id, resolve) {
  scWidget = SC.Widget(
    iframes[SoundCloud].find("iframe")
      .attr({
        scrolling: "no",
        frameborder: "no",
        playsinline: 1,
...
function setVolume() {
  ...
  switch (whichVideo) {
    ...
    case Nico:
      return postNico({ eventName: "volumeChange", data: { volume: v / 100 } });
    case SoundCloud:
      return scWidget.setVolume(v);
  }
}
function play() {
  switch (whichVideo) {
    case YouTube:
      return g_yt.playVideo();
    case Nico:
      return postNico({ eventName: "play" });
    case SoundCloud:
      return scWidget.play();
  }
}
function pause() {
  switch (whichVideo) {
    case YouTube:
      return g_yt.pauseVideo();
    case Nico:
      return postNico({ eventName: "pause" });
    case SoundCloud:
...
function seekTo0() {
  ...
  switch (whichVideo) {
    case YouTube:
      return g_yt.seekTo(s);
    case Nico:
      return postNico({ eventName: "seek", data: { time: s * 1000 } });
    case SoundCloud:
      return scWidget.seekTo(s * 1000);
  }
}
